...
pub fn ansi256_from_rgb(rgb_color: RgbColor) -> Ansi256Color {
...
fn cube_index(rgb_color: RgbColor) -> (/* cube_index */ u8, /* cube_rgb */ RgbColor) {
    let RgbColor {
        red: r,
        green: g,
        blue: b,
...
/// From: <https://www.compuphase.com/cmetric.htm>. Calculates distance between two
/// colors `d(x, x) = 0` and `d(x, y) < d(x, z)` implies `x` being closer to `y` than to
/// `z`.
fn distance(this: RgbColor, other: RgbColor) -> u32 {
    let RgbColor {
        red: x_red,
        green: x_green,
        blue: x_blue,
