        ...
        tokio::select! {
            ...
            result /*: Result<(TcpStream, SocketAddr), Error> */ = listener.accept() => {
                ...
                tokio::spawn(async move {
                    ...
                    let result = handshake::try_accept_or_timeout(&mut read_half, &mut write_half).await;
                    match result {
                        Ok(_) => tracing::info!("Handshake successful"),
                        Err(err) => tracing::warn!("Handshake failed due to: {}", err.root_cause()),
                    }
