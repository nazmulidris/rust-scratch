...
/// a "logical" cursor that the user can move by pressing up, down, left, right, etc. For left, this
/// is assumed to move the caret or cursor one position to the left. Let's unpack that.
///
...
///    boundaries.
///
/// To complicate things further, the size that a grapheme cluster takes up is not the same as its
/// byte size in memory. Let's unpack that.
///
...
/// the string (which isn't byte boundary based) as a "physical" index into the rendered output of
/// the string in a terminal.
///
